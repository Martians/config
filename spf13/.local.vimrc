...
" 显示
    ...
    " let g:loaded_litecorrect = 0  " 不进行纠错，以免在注释中显示很多的 '补丁'；此配置无效 
    set nospell                     " 注意：要把 vimview 等文件夹都清理掉，否则这些文件还是使用是之前的配置
...
" 习惯
    " let g:spf13_no_autochdir = 1      " 此处无效，必须在.vimrc.before，不改变当前路径，放在before中
...
" 编码
    " export LC_ALL=en_US.UTF-8           " ~/.bashrc，保持 iterm2 与目标系统的编码保持一致
